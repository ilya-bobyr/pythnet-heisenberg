...
pub struct AddProductArgs {
    ...
    ///
    /// You can repeat this argument, to create multiple products in parallel in a single
    /// invocation.  Note that the `--metadata` arguments need to be indexed in this case.
...
pub fn per_product_metadata<'source>(
    metadata: &'source [MetadataProductKeyValue],
) -> Vec<Vec<MetadataKeyValueRef<'source>>> {
