...
impl<'rpc_client> RunWithBlockhashArgs<'rpc_client> {
    ...
    pub fn run<'context, T, Op, OpRes>(
        self,
        op: Op,
    ) -> impl Future<Output = T> + 'rpc_client + 'context
    where
        Op: FnOnce(/* blockhash_cache: */ BlockhashCache) -> OpRes + 'rpc_client + 'context,
        OpRes: Future<Output = T>,
        'rpc_client: 'context,
    {
        let Self {
            rpc_client,
        ...
        async move {
            ...
            let op_task = op(blockhash_cache.clone());
            pin!(op_task);
