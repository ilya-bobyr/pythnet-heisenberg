...
pub async fn run(
    ...
    let payer = payer_keypair
        ...
        .transpose()?;
    let payer = payer.as_ref().unwrap_or_else(|| &signer);
    let payer_pubkey = payer.pubkey();

    let from = from_keypair
        .map(|keypair| read_keypair_file(keypair))
        .transpose()?;
    let from = from.as_ref().unwrap_or_else(|| payer);
    ...
    with_sheppard(rpc_client)
        .run(actions.iter().filter_map(|action| {
            if action.add_lamports == 0 {
                None
            } else {
                Some(fill_up_tx(
                    &signer,
        ...
        .await
        .with_context(|| format!("Running transfer transactions"))?;
...
async fn from_account_has_enough_balance(
    ...
    let Some(Account { lamports, .. }) = account else {
        println!("From account ({from}) does not exist");
        return Ok(false);
    };
