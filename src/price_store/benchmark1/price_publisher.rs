...
use super::{PriceUpdateResult, RunStats, price_source::PriceSource};
...
fn start_all_price_updates<'update_deps, 'rpc_client: 'update_deps, 'socket: 'update_deps>(
    ...
    let price_batches = price_sources
        .chunks(price_updates_per_tx.into())
        .into_iter()
        .map(|chunk_price_sources| {
            chunk_price_sources
                .iter()
                .map(|price_source| {
